...
public class ContestService{
    ...
    public Contestant createContestant(Long contestId, String userName) {
        ...
        // Validate if the contest and user exists otherwise throw RunTimeException with an appropriate message.
        if(contest == null || user == null) throw new RuntimeException("Contest or User doesn't exist");
        // Validate if the contest is has not yet started / ended otherwise throw RunTimeException with an appropriate message.
        if(contest.getContestStatus().equals(ContestStatus.ENDED)) throw new RuntimeException("Contest have ended.");
        if(contest.getContestStatus().equals(ContestStatus.IN_PROGRESS)) throw new RuntimeException("Contest in progress.");
        // Create and store Contestant in the contestantRepository.
        contestantRepository.save(new Contestant(user,contest));
        return contestantRepository.find(contestId, userName).get();
    //  return null;
    }
    ...
    public List<Contestant> contestHistory(Long contestId) {
        Contest C = contestRepository.findById(contestId).get();
        if(C.getContestStatus().equals(ContestStatus.ENDED)){
            return contestantRepository.findAllByContestId(contestId).stream().sorted(Comparator.comparingInt(a -> -a.getCurrentContestPoints())).collect(Collectors.toList());
        } throw new RuntimeException("Contest has not ended yet!");
    //  return Collections.emptyList();
    }  
}
